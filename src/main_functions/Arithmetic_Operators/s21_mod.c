...
int s21_mod(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (is_less_no_sign(ext_value_1, ext_value_2) &&
      is_equal_no_sign(tmp, ext_result)) {
    ...
    if (!is_less_no_sign(ext_value_1, ext_value_2)) {
      step_to_shift = get_first_non_zero_extBit(ext_value_1) -
                      get_first_non_zero_extBit(ext_value_2);
      shift_left(&ext_value_2, step_to_shift);
      if (is_less_no_sign(ext_value_1, ext_value_2))
        shift_right(&ext_value_2, 1);
      if (!is_less_no_sign(ext_value_1, ext_value_2) ||
          is_equal_no_sign(ext_value_1, ext_value_2))
  ...
  if (error_nan != 3) {
    while (1) {
      if (!is_less_no_sign(tmp, ext_value_2) ||
          is_equal_no_sign(tmp, ext_value_2)) {
        step_to_shift = get_first_non_zero_extBit(tmp) -
                        get_first_non_zero_extBit(ext_value_2);
        shift_left(&ext_value_2, step_to_shift);
      ...
      if (is_less_no_sign(tmp, ext_value_2)) {
        step_to_shift = get_first_non_zero_extBit(ext_value_2) -
                        get_first_non_zero_extBit(tmp);
