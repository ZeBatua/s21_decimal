...
int s21_add(s21_decimal value_1, s21_decimal value_2,
            s21_decimal *result) {  // ничего не записывается в 3 бит
  int error = 0;
  ...
  // в зависимости от указанного скейла можно узнать бит переполнения. То есть
  // scale = 0 - 191;
  ...
  if (dec1_sign == dec2_sign) {
    ...
    setExtScale(getExtScale(ext_value_1), &ext_result);
                 ...
                 ext_value_1,
                 ext_value_2)) {  // иф для а - а = 0 // перепроверит equal //
                                  // на это вообще мб забить
  ...
  // printf("last print in s21_add\n");
  // smart_print_exdec(ext_result);

  error = equate_extdec_to_dec(
      ext_result, result);  // по идее надо округлять прежде чем приравнивать
  return error;
}
