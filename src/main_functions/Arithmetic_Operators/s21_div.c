...
int s21_div(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) { // сделать обработку знаков в начале и в конце
    ...
    if (is_equal_no_sign(ext_value_2, zero)) {
        ...
        while (bug_stop < 100) {
            decrease_divinded_length = subtraction_for_div(&ext_value_1, ext_value_2, &subtraction_times); // try to substract
            divinded_length -= decrease_divinded_length;

            if (divinded_length > 1) {
                multiply_extdec_by_ten(&ext_value_1);
                setExtScale(getExtScale(ext_value_1) - 1, &ext_value_1);
                set_next_digit(&ext_value_1, buf_value_1, next_digit_counter);
                next_digit_counter--;
            } else if (divinded_length <= 1 && !is_equal_no_sign(ext_value_1, zero)) {
                multiply_extdec_by_ten(&ext_value_1);
                setExtScale(getExtScale(ext_value_1) - 1, &ext_value_1);
            }
            ...
            if (divinded_length <= 1 && is_equal_no_sign(ext_value_1, zero) && scale_length > 0) {
                scale_length++,
                setExtScale(scale_length, &subtraction_times);
                break;
            }
            if (divinded_length <= 1 && is_equal_no_sign(ext_value_1, zero)) break;

            if (divinded_length <= 1 && f_width != 1) scale_length++, setExtScale(scale_length, &subtraction_times);
            if (scale_length > 28) break;
            bug_stop++;
