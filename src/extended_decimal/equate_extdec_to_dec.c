...
int equate_extdec_to_dec(s21_extended_decimal E_decimal, s21_decimal *decimal) {
  ...
  if (check_E_decimal.extBits[5] != 0 || check_E_decimal.extBits[4] != 0 ||
  ...
  if (error == 0) {
    ...
    if (bank_E_decimal.extBits[0] % 2 == 1 &&
        plus_one == 1) {  // decimal НЕчетный, remainder > 5
      add_no_equote(E_decimal, one, &E_decimal);
    } else if (bank_E_decimal.extBits[0] % 2 == 1 && plus_one == 0 &&
               five == 0) {  // decimal НЕчетный, remainder < 5
                             // nichigo
    } else if (bank_E_decimal.extBits[0] % 2 == 1 && plus_one == 0 &&
               five == 1) {  // decimal НЕчетный, remainder == 5
      add_no_equote(E_decimal, one, &E_decimal);
    } else if (bank_E_decimal.extBits[0] % 2 == 0 && plus_one == 1 &&
               five == 0) {  // decimal четный, remainder > 5
      add_no_equote(E_decimal, one, &E_decimal);
    } else if (bank_E_decimal.extBits[0] % 2 == 0 && plus_one == 0 &&
               five == 0) {  // decimal четный, remainder < 5
                             // nichigo
    } else if (bank_E_decimal.extBits[0] % 2 == 0 && plus_one == 0 &&
               five == 1) {  // decimal четный, remainder == 5
                             // nichigo
    }

    //------end_bank_round-----//

    if ((E_decimal.extBits[3] != 0 || E_decimal.extBits[4] != 0 ||
         E_decimal.extBits[5] != 0) &&
        getExtSign(E_decimal) == 0) {
      error = 1;
    } else if ((E_decimal.extBits[3] != 0 || E_decimal.extBits[4] != 0 ||
                E_decimal.extBits[5] != 0) &&
